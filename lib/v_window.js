...
function bindWindowTrigger(el, binding) {
  ...
  if (!globalTriggerNode) {
    ...
    globalTriggerNode.style =
      "width: 20px; height: 20px; background-color: #eee; border-radius: 5px; position: fixed; z-index: 99;";
  ...
  const triggerFunc = function () {
    ...
    el.className += " v-windowed";
    currentTriggerNode.style.display = "none";
    vWindow.mount();
  };
  el.addEventListener("mouseenter", function (e) {
    if (e.target.className.indexOf("v-windowed") > -1) {
      return;
    }
...
function createWindow(windowName) {
  ...
  windowNode.style =
    "height: 25px; width: 100px; position: fixed; border: 1px solid #666; background-color: #fff; display: flex; flex-direction: column; z-index: 999;";
  ...
  return {
    ...
    setContentNode(node) {
      ...
      closeButtonNode.addEventListener("click", (e) => {
        e.stopPropagation();
        this.nodeInfo.node.className = this.nodeInfo.node.className.replace(
          "v-windowed",
          ""
        );
